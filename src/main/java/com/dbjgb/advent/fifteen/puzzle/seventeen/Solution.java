package com.dbjgb.advent.fifteen.puzzle.seventeen;
...
public class Solution {
  ...
  private static final List<Integer> BUCKET_SIZES = Lists.newArrayList();
  private static final Multimap<Integer, Integer> SUMS_MAP = ArrayListMultimap.create();

  public static void main(String... args) throws Exception {
    try (BufferedReader bufferedReader =
        Utility.openInputFile("fifteen/puzzle/seventeen/input.txt")) {
      String line;
      while ((line = bufferedReader.readLine()) != null) {
        BUCKET_SIZES.add(Integer.valueOf(line));
      }
    }

    int sign = 1;
    Collections.sort(BUCKET_SIZES);
    if (150 < 0) {
      Collections.reverse(BUCKET_SIZES);
      sign = -1;
    }
    System.out.println(BUCKET_SIZES);

    SUMS_MAP.put(0, -1);
    for (int index = 0; index < BUCKET_SIZES.size(); index++) {
      Set<Integer> sums = Sets.newHashSet(SUMS_MAP.keys());
      for (Integer sum : sums) {
        int newSum = sum + BUCKET_SIZES.get(index);
        if (0 < (newSum - 150) * sign) {
          continue;
        } else {
    ...
    Set<Set<Integer>> combinations = findCombinations(150);
  ...
  public static void findCombinations(int target, Set<Set<Integer>> accumulator, Set<Integer> path) {
    ...
    for (int index : SUMS_MAP.get(target)) {
      ...
      int newTarget = target - BUCKET_SIZES.get(index);
      if (newTarget > 0) {
